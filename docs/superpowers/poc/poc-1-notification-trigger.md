...
6. **테스트 문구는 합성만** 쓴다. 본인 두 번째 계정·채널에서 "[합성] 9월 25일 15시 예약 확정" 같은 문구를 보낸다. 실제 알림톡·대화 원문을 쓰지 않는다(AGENTS §7).
...
- 연락처 발신자 폐기 규칙은 아직 배선되지 않았다(`RuleFilter()`에 연락처 목록을 넘기지 않음). 이 PoC에서는 관찰 대상이 아니다.
