...
| PoC-6 | Gmail watch → Pub/Sub → history 동기화 | 10 | 부분(서버 경로 모의 테스트 통과, 실계정 연결 미검증) | 09-27 서버 구현·배포(`0006_gmail`, `gmail-connect`·`gmail-webhook`·worker 잡 3종). 모의 API 테스트 16개: history 페이지 병합, **404 → profile 후 `messages.list(after: last_success_at-1일)` 재동기화**, `invalid_grant` → `ReauthRequired` → `reauth_required`·잡 `skipped`, watch 갱신·만료 기록, 규칙 필터(프로모션·OTP 폐기, 카드 마스킹) → encrypt → `insert_item`, connect 흐름, OIDC(aud·SA 이메일·email_verified·issuer·만료·서명 키). DB 3개: cron 적재 중복 없음, `gmail_reauth_due`, vault refresh token 소유자 한정·삭제 트리거. 배포 실측: webhook은 OIDC 없는 요청 모두 401, connect는 인증 없음·publishable 401, 사용자 JWT+가짜 코드 502(Google 도달), Web 클라이언트 자격은 Google이 수락(`invalid_grant`). 남은 실측: Push 구독 생성·`PUBSUB_PUSH_SA_EMAIL` 등록, iOS GoogleSignIn 연결, 백필 건수·시간·429, 웹훅 지연 5회, 404 재동기화 누락 0, watch 갱신, 6일/8일 재인증(절차 `poc-6-gmail.md`) | 2026-09-27 |
