...
**Files:**
- Create: `poc/server/supabase/config.toml`(CLI 생성), `poc/server/supabase/migrations/0001_jobs.sql`, `poc/server/supabase/migrations/0002_cron.sql`, `poc/server/supabase/migrations/0003_items_keys.sql`, `poc/server/supabase/functions/_shared/job.ts`, `poc/server/supabase/functions/_shared/crypto.ts`, `poc/server/supabase/functions/worker/index.ts`, `poc/server/supabase/functions/worker/process.ts`, `poc/server/supabase/tests/jobs.test.ts`, `poc/server/supabase/tests/crypto.test.ts`, `poc/server/scripts/seed-items.ts`, `poc/server/.env.example`
...
- Produces:
  ...
  - 함수 `insert_item(p_user, p_source, p_idempotency_key, p_sender, p_title, p_content_enc bytea, p_occurred_at, p_enqueue boolean default true) returns uuid`(중복이면 null, `p_enqueue`면 `jobs(kind='process')` 동시 삽입), `worker_get_item(p_user uuid, p_item uuid) returns table(id, content_enc, ocr_text_enc)`(소유자 불일치면 0행, 반환 시 `audit_log(action='decrypt')`), `get_wrapped_key(p_user) returns bytea`, `put_wrapped_key(p_user, p_wrapped bytea) returns bytea`
...
```
`.env.example`: `SUPABASE_URL=`, `SUPABASE_SERVICE_ROLE_KEY=`, `MASTER_KEY=`, `POC_USER_ID=`, `ANTHROPIC_API_KEY=`, `VOYAGE_API_KEY=`, `APNS_KEY_ID=`, `APNS_TEAM_ID=6626BYCJG4`, `APNS_P8=`, `APNS_TOPIC=com.picpal.assistant.poc`, `GOOGLE_CLIENT_ID=`, `GOOGLE_CLIENT_SECRET=`, `PUBSUB_SA_EMAIL=`, `GMAIL_PUBSUB_TOPIC=`.

PoC 사용자 1명 생성(`items.user_id`가 `auth.users`를 참조하므로 필요). 출력된 UUID를 `.env`의 `POC_USER_ID`에 넣는다:
```bash
deno eval --env-file=.env 'import { createClient } from "npm:@supabase/supabase-js@2";
const sb = createClient(Deno.env.get("SUPABASE_URL"), Deno.env.get("SUPABASE_SERVICE_ROLE_KEY"));
const { data, error } = await sb.auth.admin.createUser({ email: "poc-user@example.com", email_confirm: true });
...
declare v_id uuid;
begin
  insert into items (user_id, source, sender, title, content_enc, occurred_at, idempotency_key)
  values (p_user, p_source, p_sender, p_title, p_content_enc, p_occurred_at, p_idempotency_key)
...
revoke execute on function get_wrapped_key(uuid), put_wrapped_key(uuid, bytea),
  insert_item(uuid, text, text, text, text, bytea, timestamptz, boolean), worker_get_item(uuid, uuid)
...
- [ ] **Step 6: worker Edge Function (process 단계는 복호화를 거친다)**
...
6. 24시간 두고 프로젝트가 일시정지되지 않는지 확인(무료 티어 활동 유지).

- [ ] **Step 8: 커밋** `git commit -m "poc(server): supabase project, jobs queue, envelope encryption, worker edge function"`
...
**Interfaces:**
- Consumes: Task 8의 `items`·`insert_item`·`enqueue_job`·`encrypt`·`toBytea`·`Job`·worker `handlers`.
- Produces:
  - `connections(id, user_id, provider, account_ref, status, expires_at)`, `sync_states(connection_id pk, user_id, cursor, last_success_at, watch_expires_at)`. refresh token은 `vault`에 `gmail_rt:<connection_id>` 이름으로 저장(스펙 §7).
  - 저장 프로시저: `gmail_save_connection(p_user, p_account_ref, p_refresh_token, p_history_id) returns uuid`, `gmail_get_refresh_token(p_user, p_connection) returns text`, `gmail_state(p_user, p_connection) returns table(cursor, last_success_at)`, `gmail_update(p_user, p_connection, p_cursor, p_status, p_watch_expires_at)`, `gmail_enqueue_for_account(p_account_ref text) returns uuid`, `gmail_enqueue_all(p_kind text) returns int`.
  - `_shared/gmail.ts`: `exchangeCode`, `refreshAccessToken(refreshToken): Promise<string>`(invalid_grant → `ReauthRequired`), `watch`, `profile`, `listMessageIds`, `history`, `getMessage`, `plainText(msg)`, `header(msg, name)`, `collectNewMessageIds(api: GmailApi, state: { cursor: string; lastSuccessAt: string }): Promise<{ ids: string[]; cursor: string; mode: "history" | "resync" }>`.
  - worker 잡 종류: `gmail-sync`(history → 404면 `messages.list(after:)` 재동기화), `gmail-fetch`(본문 → `encrypt` → `insert_item`), `gmail-watch`(매일 watch 갱신). 모두 `lease_key = 'gmail:' + connection_id`.
...
revoke execute on function gmail_save_connection(uuid, text, text, text), gmail_get_refresh_token(uuid, uuid),
  ...
  gmail_enqueue_all(text) from public, anon, authenticated;
...
```
watch는 7일 뒤 만료되므로 매일 갱신해 하루 이상 실패해도 끊기지 않게 한다(스펙 §7 "매일 watch 갱신").
...
import { createClient } from "npm:@supabase/supabase-js@2";
import { collectNewMessageIds, type GmailApi, plainText, ReauthRequired, refreshAccessToken } from "../functions/_shared/gmail.ts";
...
```
실행: `cd poc/server && deno test --allow-net --allow-env --allow-read --env-file=.env supabase/tests/gmail.test.ts` → `gmail.ts`가 없어 실패. 마지막 테스트는 `supabase db push` 후 통과한다(Task 8처럼 워커 cron을 잠시 멈춘다).
...
```
실행: `deno test ... supabase/tests/gmail.test.ts` → 5개 통과.
...
import { collectNewMessageIds, getMessage, gmailApi, header, plainText, ReauthRequired, refreshAccessToken, watch } from "./gmail.ts";
...
export async function gmailFetch(sb: SupabaseClient, job: Job): Promise<string> {
  ...
  for (const id of job.payload.ids as string[]) {
    ...
    const { error } = await sb.rpc("insert_item", {
      ...
      p_sender: header(m, "From"), p_title: header(m, "Subject"),
      p_content_enc: toBytea(await encrypt(user, plainText(m))),   // 평문 본문은 DB로 가지 않는다
      p_occurred_at: new Date(Number(m.internalDate)).toISOString(),
    });
    if (error) throw new Error("insert_item " + error.code);
...
```
`gmail-fetch`가 `format=full`로 본문을 받아 `plainText` → `encrypt` → `insert_item`(process 잡 동시 생성)으로 저장한다. 제목·발신자는 스펙 §8대로 평문 컬럼이다.
...
6. 8일 후 refresh 실패 재현: 테스트 사용자 동의 7일 만료 → `connections.status = reauth_required`로 전환되고 이후 gmail 잡이 `skipped`로 끝나는지. (달력에 표시하고 8일 뒤 확인)
...
1. Task 8: `supabase projects create`(org 선택), `supabase link`, `MASTER_KEY` 생성(`openssl rand -base64 32`)을 `.env`와 `supabase secrets set`에, PoC 사용자 생성 후 `POC_USER_ID`를 `.env`에.
