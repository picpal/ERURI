...
> **임베딩 보류 (스펙 §12 통제 3·§16):** 이 태스크는 **합성 코퍼스(`eval/corpus.jsonl`)만** Voyage로 임베딩하는 평가 태스크다. Gmail 백필·공유 항목 등 실제 사용자 데이터에는 `embed()`를 호출하지 않고, worker의 `process` 경로에도 임베딩을 연결하지 않는다. **PoC-7이 통과해도 프로덕션 임베딩은 Voyage 약관 확인(§16) 전까지 활성화하지 않는다.** 그동안 검색은 `hybrid_search(p_embedding => null)` 키워드 경로만 쓴다.

**Files:**
- Create: `migrations/0004_hybrid_search.sql`, `functions/_shared/voyage.ts`, `eval/corpus.jsonl`, `eval/questions.jsonl`, `eval/run-search-eval.ts`, `tests/search.test.ts`

**Interfaces:**
- Consumes: Task 8의 `items`·`insert_item`·`encrypt`·`toBytea`, `.env`의 `POC_USER_ID`.
- Produces: `item_chunks(id, item_id, user_id, chunk_index, text, embedding vector(512) null, tsv tsvector)`, 함수 `hybrid_search(p_user uuid, p_query text, p_embedding vector(512), p_limit int, p_from timestamptz default null, p_to timestamptz default null) returns table(item_id uuid, chunk_id uuid, score float)` (RRF, k=60, pg_trgm 유사도 0.2 이상도 후보 포함. `p_embedding`이 null이면 키워드·trigram만으로 순위). `embed(texts: string[], inputType: "document"|"query"): Promise<number[][]>`.
...
create table item_chunks (
  ...
  text text not null,
  embedding vector(512),                  -- Voyage 약관 확인 전 실제 데이터는 null로 둔다
...
- [ ] **Step 2: 실패하는 테스트 — Voyage 클라이언트, 임베딩 없는 검색**
...
import { createClient } from "npm:@supabase/supabase-js@2";
import { encrypt, toBytea } from "../functions/_shared/crypto.ts";
import { embed } from "../functions/_shared/voyage.ts";
const sb = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
const USER = Deno.env.get("POC_USER_ID")!;
...
```
실행: `cd poc/server && supabase db push && deno test --allow-net --allow-env --allow-read --env-file=.env supabase/tests/search.test.ts` → `voyage.ts`가 없어 실패.

- [ ] **Step 3: Voyage 클라이언트 구현**

`_shared/voyage.ts`:
```ts
// 스펙 §12 통제 3: Voyage 약관 확인 전에는 합성 코퍼스(eval)에만 호출한다
...
`run-search-eval.ts`: 코퍼스를 적재한다 — 각 건을 `encrypt(POC_USER_ID, text)` → `insert_item(p_enqueue => false, p_idempotency_key => 'eval:' + id)`로 `items`에 넣고(process 잡을 만들지 않아 Haiku 호출 없음), 512자 청크를 `item_chunks`에 넣으며 `embed(..., "document")` 배치 64로 `embedding`을 채운다. 이어서 각 질문을 두 방식으로 돌린다:
- **하이브리드**: `embed([q], "query")` → `hybrid_search` Top-5
- **키워드 전용**: `hybrid_search(p_embedding => null)` Top-5 — Voyage 확인 전 실제로 쓰일 경로의 기준선
...
`deno run --allow-net --allow-env --allow-read --allow-write --env-file=.env eval/run-search-eval.ts`. 미달 시 청크 256자, `output_dimension` 1024, 키워드 가중치 변경을 각각 1회씩 시도하고 표에 나란히 기록. results.md PoC-7 행에 "통과해도 프로덕션 임베딩은 Voyage 약관 확인(§16) 전까지 비활성"을 함께 적는다. `git commit -m "poc(server): hybrid search (null-embedding path) + synthetic Korean eval corpus"`
...
```ts
import { extractEvent } from "../_shared/anthropic.ts";
// handlers 객체 안
...
Task 11은 Task 8의 `items`·`insert_item`·`crypto.ts`에 의존하므로 Task 8 완료 뒤 시작한다. Task 11은 합성 코퍼스만 임베딩한다. **PoC-7 통과 후에도 프로덕션 임베딩은 Voyage 약관 확인(스펙 §16) 전까지 활성화하지 않는다.**
