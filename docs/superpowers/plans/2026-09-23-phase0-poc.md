...
> **구현 결과 (완료, 연락처 로딩 제외):** 커밋 `964cfea`, 리뷰 반영 `0d2a293`. 원안과 차이: `text` 기본값 `""`(본문이 비어 와도 단축어가 값을 묻지 않음), 로그에 값 없이 필드 메타(`app=` `titleLen=` `textLen=` `sender=set|nil` `locked=`), 오류도 로그 후 throw, 규칙 폐기 사유(`discarded:otp`/`contact`) 유지, BFU 전용 로그(`BFULog`, 보호 등급 none, 길이만). Step 3(단축어 앱 수동 실행)은 XCUITest `testShortcutsAppRunsCaptureIntent`로 수행(`CaptureIntent queued:rules … textLen=0`). 판정 상태: PoC-1/2 **미검증**(실기기 필요). 연락처 로딩은 Step 6(미완).
...
> **구현 결과 (완료):** 커밋 `f28814d`, 리뷰 반영 `0d2a293`. 원안과 차이: 목 서버 포트를 `MOCK_INGEST_PORT`(기본 8787)로 바꿀 수 있게 했고, 이 기계는 8787·8788이 무관한 프로세스에 점유돼 **8799**로 실측했다(`SIMCTL_CHILD_INGEST_URL=http://localhost:8799`). `MOCK_INGEST_SLOW_MS`(기본 0)는 업로드 바디를 청크마다 늦게 읽어 "전송 도중 앱 종료"를 재현하는 PoC-9 전용 옵션. `Uploader`는 `@unchecked Sendable` 없이 `let session` + 별도 `UploadDelegate`, flush는 `pending` 대신 `claim`(lease)으로 중복 업로드를 막는다. 판정 상태: PoC-9 **통과**(앱 프로세스 종료 확인 후 3.68초 뒤 정확한 바이트 도착), PoC-8 기기 부분 **부분**(사진 앱 공유 시트 XCUITest `testPhotosShareSheetRunsExtension`, `ocrLen=43`). 남은 일: Share Extension이 규칙 필터를 적용하지 않는다(스펙 §16 코드 수정 필요 #2).
...
| `--poc-debug-capture-intent=<app>` | 앱 프로세스에서 `CaptureIntent.perform()` 전체(규칙 → FM → 라우팅 → 큐) | 단축어 호출 경로는 아님 |
...
| PoC-7 | 검색 (합성 코퍼스만) | 서버 | 하이브리드 Top-5=, 키워드 전용 Top-5=, 거절률=, 날짜오판= | ≥90%, ≥90%, 0 | | 프로덕션 임베딩은 Voyage 약관 확인(§16) 전까지 비활성 | |
| PoC-8 | 이미지 추출 | 시뮬 + 서버 | 5종 적중, OCR 대안 적중 | 5/5 | | | |
| PoC-9 | background upload | 시뮬 | 오프라인·강제종료 유실 | 0 | | | |
| PoC-10 | jobs 워커 | 서버 | 중복 실행, dead 전환, decrypt_ms p95=, 배치 크기= | 0, 5회 후 dead, CPU 2초 이내 | | | |
```
