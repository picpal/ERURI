...
| 태스크 구현 (TDD, Swift/TS 코드, 마이그레이션) | `opus` | `high` | 계획서에 코드가 있어도 동시성 오류·플랫폼 제약 판단이 매번 필요 |
| PoC 실기기·시뮬레이터 시나리오 실행과 기록 | `opus` | `medium` | 실측 게이트의 통과/부분 판정이 정확해야 함 |
| 리포트·문서 작성 (HTML 리포트, 절차서) | `opus` | `medium` | 문서 간 불일치를 잡아내야 함 |
...
규칙:
- **기본값은 `opus`(Opus 5.5)**. 사용자 지시(2026-09-24): 판단이 섞이는 작업은 웬만하면 opus. sonnet·haiku는 반복·조회 작업에만.
